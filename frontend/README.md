...
# IoT Market Segmentation and Positioning Frontend

This frontend is part of a GenAI-driven multi-agent system for market segmentation and strategic positioning in IoT markets. The system is designed for research and evaluation as part of a Master's thesis project.

## Context

- **Purpose:** Enable interactive exploration and visualization of IoT market segmentation and positioning results generated by the backend multi-agent pipeline.
- **System:** The backend coordinates four agents (IoT Vertical, Geo Segmentation, Segment, Positioning) and uses Retrieval-Augmented Generation (RAG) to ground outputs in high-quality, manually curated sources.
- **Workflow:** User queries and context are processed through the agent pipeline, with each agent contributing structured insights to the final report.
- **Research Focus:** Actionable, transparent, and auditable segmentation and positioning for IoT vendors, with explicit scoring and recommendations.

## How it works

- The frontend displays results and recommendations from the backend pipeline.
- All outputs are synthetic and for illustrative purposes only.
